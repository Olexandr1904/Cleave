...
| 2026-05-12 | Replaced singular `_tracker: TrackerInterface | None` with `_trackers: dict[str, TrackerInterface]` keyed by project_id. Added `register_tracker`, `get_tracker_for_project`, `get_tracker_for_workspace` accessors. `set_tracker` removed. Runtime `get_tracker` callback renamed to `get_trackers`; poll-cycle guard now checks non-empty dict. main.py registers the first-project tracker post-construction; `on_project_added` registers per-project tracker. TODO shims in `_poll_and_create_workspaces` and `analyze_ticket_ids` preserved for Task 3. |
| 2026-05-12 | `poll_and_create_workspaces` (ingest.py) now accepts `trackers: dict[str, TrackerInterface]` instead of a single `tracker`. Each project's tracker is looked up per-project; failures are isolated per-tracker. `analyze_ticket_ids` similarly accepts `trackers: dict` and tries each tracker in turn (first hit wins). TODO Task 3 shims in `orchestrator.py` removed. |
| 2026-05-12 | `analyze_ticket_ids` (ingest.py): removed dead `found_project_id` variable (routing derives project from ticket labels via `route_manual_ticket`, not from this var); renamed `_create_ws` closure params from `project_id`/`repo_config` to `pid`/`repo_cfg` to eliminate shadowing of the outer loop variable; updated docstring to reflect the current tracker-loop flow. |
...
| 2026-05-14 | `_execute_cli` now passes `add_dirs=[str(workspace.meta_dir)]` to `adapter.execute_in_workspace`, exposing the ticket `meta/` directory to the CLI agent via `--add-dir`. The CLI runs with `cwd=source/` so without this the agent could not read attachments or comments via its own file tools. |
| 2026-05-14 | `assemble_prompt` stops inlining attachment files. A new `_build_attachments_manifest` helper emits a `## Ticket Attachments` manifest of absolute paths instead — truncated crash logs are noise, and images are not text. Meta files (ticket.md, comments.md, …) are still inlined but now append `full file at <path>` when truncated so agents can read the rest via tools. |
...
- `pipeline/actions/push_and_open_pr.py`: extracted `_action_push_and_open_pr` + 3 git helpers (`_commit_pipeline_artifacts`, `_ensure_branch_has_commits`, `_squash_feature_commits`). All four kept as Orchestrator shims for callers/tests.
- `pipeline/actions/fetch_pr_comments.py`: extracted the PR-review escalation loop — `_action_fetch_pr_comments`, `_reinvestigate_pending`, `_execute_review_decisions`, `_send_escalated_comment_tg` (~520 LOC, largest single move). Shims pass `get_vcs` / `get_chat_id` as thunks so test fixtures that mock `Orchestrator` without `_projects` / `_get_vcs_for_workspace` keep working. Behavior pinned by Phase-0 characterization tests.
- `pipeline/actions/finalize.py`: extracted `_action_finalize` and `_on_ticket_done`. Last leaf extraction of Phase D — `orchestrator.py` is now down to the pipeline driver, state-machine plumbing, and a thin DI surface.
- `pipeline/agent_stage.py`: extracted `_handle_agent_stage` (~170 LOC main loop) + `_rollback_iteration` + `_parse_agent_outcome` + `_looks_like_pass`/`_looks_like_fail` keyword matchers. The biggest single extraction by parameter count — the function takes orchestrator-side callbacks for `advance_to_stage`, `on_ticket_done`, `build_gate_summary`, `notify_verification_blocked`, `notify_deferred`, `notify_failed` since those still live on `Orchestrator` until later phases. `get_chat_id` is passed as a thunk so test fixtures that mock `Orchestrator` without `_global_config` keep working. Backward-compat re-exports of `_looks_like_pass` / `_looks_like_fail` are kept on `orchestrator.orchestrator` for existing tests.
- `pipeline/action_stage.py`: extracted `_handle_action_stage` dispatcher. Routes to extracted action functions (push_and_open_pr, fetch_pr_comments, finalize) via callbacks since those still live as Orchestrator shims.
- `pipeline/driver.py`: extracted the state-machine driver — `advance_workspace`, `_advance_to_stage`, `_build_gate_summary`, plus the `_STAGE_TO_STATE` / `_STATE_TO_STAGE` mappings. Pipeline core is now self-contained; `Orchestrator` keeps thin shims.
- `ingest.py`: extracted `_poll_and_create_workspaces`, `_create_workspace_for_ticket`, `_route_manual_ticket`, `analyze_ticket_ids` (4 methods). Tracker, projects dict, workspace manager, default_model_provider, and repo_vcs flow through as explicit params — no more `self._tracker` reads in the ingest path. `_create_workspace_for_ticket` is passed back through the Orchestrator shim as `create_workspace_fn` so existing tests that patch `orc._create_workspace_for_ticket` keep working without changes.
- `runtime.py`: extracted `run`, `poll_cycle`, `_reconcile_disk_workspaces`, `_sweep_deferred`, `shutdown`, `_handle_shutdown`, `_emit` into a new `Runtime` class. Runtime owns the daemon's long-running state (active workspaces, recent completions, semaphore, shutdown/wake events). `Orchestrator` wraps a `Runtime` and delegates lifecycle methods. Properties on `Orchestrator` forward to the runtime so existing private-attribute access patterns keep working — including tests that build the orchestrator via `__new__` and assign `_active_workspaces` / `_shutdown_event` directly (the properties fall back to `__dict__` when `_runtime` is absent, and `_reconcile_disk_workspaces` / `_sweep_deferred` lazily construct a Runtime on demand for those fixtures). Per-cycle callbacks (`_poll_and_create_workspaces`, `advance_workspace`, `_rescan_projects_from_disk`) are passed as thunks so tests can patch the bound methods after construction.

## Shim cleanup pass (2026-05-12)

After the layer extractions above, `orchestrator.py` was 736 LOC, mostly pass-through methods that delegated to the new modules. This pass removed the trivial ones; tests now import the module functions directly instead of going through `Orchestrator._foo`. End state: 522 LOC.

- Deleted from `orchestrator.py`:
  - Git/ticket: `_git_diff_files`, `_git_head_sha`, `_refetch_ticket_data`.
  - Approval/escalation: `_should_approval_gate`, `_truncate_reason`, `_build_blocked_reason`, `_handle_escalate`.
  - Driver: `_advance_to_stage`, `_build_gate_summary`, `_parse_agent_outcome`, `_rollback_iteration`.
  - Ingest: `_route_manual_ticket`. `_create_workspace_for_ticket` slimmed and renamed to `_create_workspace` (purely a DI wrapper now).
  - Notifications: `_notify_deferred`, `_notify_failed`, `_notify_rerun`, `_notify_verification_blocked`. The agent-stage path inlines these as closures inside `_handle_agent_stage`.
  - Actions: `_action_push_and_open_pr`, `_commit_pipeline_artifacts`, `_ensure_branch_has_commits`, `_squash_feature_commits`, `_action_fetch_pr_comments`, `_reinvestigate_pending`, `_execute_review_decisions`, `_send_escalated_comment_tg`, `_action_finalize`. The action-stage path builds closures inline; the `action_*` module functions are imported at module level so tests can `patch("orchestrator.orchestrator.action_push_and_open_pr", ...)`.
  - Misc: `_now`, `_log_pipeline`, `DEFAULT_QUOTA_RETRY_DELAY` (all unused).
- Kept: `_handle_agent_stage`, `_handle_action_stage`, `_on_ticket_done`, `_poll_and_create_workspaces`, `_create_workspace`, `advance_workspace`, plus the daemon-lifecycle methods on `Runtime` and the small `_get_*` helpers. These are not pass-throughs — they own the dependency-injection wiring that lets the pipeline modules stay decoupled from `Orchestrator`'s attribute soup.
- Runtime-attr properties compressed: five `@property` + setter pairs (about 75 LOC of boilerplate) replaced with a single `_RuntimeAttr` descriptor class. Same external behavior — read/write `_active_workspaces` / `_recent_completions` / `_shutdown_event` / `_wake_event` / `_agent_semaphore` forwards to `Runtime` when present, falls back to `__dict__` otherwise.
- Dashboard `actions.py` rerun handler no longer reaches into `orchestrator._refetch_ticket_data` / `orchestrator._notify_rerun`; it imports `refetch_ticket_data` and `notify_rerun` from their modules and passes `orchestrator._tracker` / `_notifier` / `_get_chat_id` explicitly.
- Tests migrated to call module functions directly across ~15 test files. The pattern: build a minimal stub or `MagicMock` carrying the deps the function reads, call the module function, assert. No behavior changes; 1046 unit tests + 4 dry-run integration tests still pass.
