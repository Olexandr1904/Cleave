...
>
> **For users:** start at the top-level [README.md](../README.md). The user-facing references for setup, dashboard, Telegram, labels, and troubleshooting live in [docs/](.). This document is a snapshot for contributors and agents who want the full architectural picture in one read.
...
**15 pipeline states:** NEW, ANALYSIS, DEV, SCOPE_CHECK, QA, PUSHED, PR_REVIEW, DONE, BLOCKED, FAILED, ARCHIVED, AWAITING_APPROVAL, MANUAL_CONTROL, DEFERRED, PAUSED
...
dashboard/
  web.py                         # Starlette/uvicorn web server
  actions.py                     # Dashboard write/action endpoints
...
6. **Source cleanup only** — `source/` (and `source/reports/` within it) deleted after merge; `meta/`, `logs/`, and `state.json` are preserved
...
### Implemented
- State machine with 15 states incl. BLOCKED/resume, DEFERRED/quota-recovery, PAUSED
...
- All 8 agent prompt files (incl. Atlas project-setup agent)
- Dashboard with structured event log, per-project ticket history, real-time refresh, project-creation wizard
- Per-project health validators wired for Jira and VCS provider (git_identity / git_remote validators exist but not yet wired into `check_project`)
- Agent permissions via `.claude/settings.json` and per-agent tool allowlists
- Per-ticket model selection via Jira label (`model-haiku` / `model-sonnet` / `model-opus`)

### Not Yet Implemented
- GitLab adapter (`integrations/gitlab/`)
- Jenkins adapter (`integrations/jenkins/`)
- CI interface (`integrations/base/ci.py`)
- Multi-stack pluggable failure recovery (currently Gradle/AAPT2-specific)
