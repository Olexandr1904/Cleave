...
| 2026-05-08 | Blocked-stage and verification-failed Telegram messages now surface the `skip` and `retry` reply keywords inline alongside "Reply with context". Previously these keywords were documented in `docs/telegram.md` but invisible at the moment the operator sees the block — they had to remember the keyword exists. Hint now lists `Reply "skip"` and `Reply "retry"` for max-iterations, agent-stage, non-agent-stage, and verification-failed paths. |
| 2026-05-14 | Manual-mode analysis approval-gate message now reads `Analysis complete — BA decision:` followed by a single sentence pulled from a new mandatory `## Verdict` section in `ba.md`. Previous behaviour grabbed an arbitrary first prose line of the `## Summary` paragraph and hard-truncated at 200 chars, often cutting mid-sentence and leaking root-cause / file-name detail. New `_extract_ba_verdict` helper in `pipeline/driver.py` prefers the Verdict section and falls back to first-sentence trim of the first prose line for plans written before the section existed. Goal: let the operator sanity-check whether the BA understood the ticket at a glance before approving start of coding. |
