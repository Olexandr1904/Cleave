...
telegram:
  chat_id: "${ACME_TELEGRAM_CHAT_ID}" # overrides global default_chat_id
...
| Situation | Message |
|---|---|
| Ticket ambiguous | `🤔 [ACME/android] ACME-123: Missing info\n\n{numbered questions}` |
| No repo label on ticket | `🏷 [ACME] ACME-123: Which repo? android-app / backend-api` |
| Scope fix loop maxed | `🔁 [ACME/android] ACME-123: Scope violations after 3 attempts. PR: {link}\n\n{violation_summary}` |
| Fix loop maxed | `🔁 [ACME/android] ACME-123: Review fixes failed after 3 attempts. PR: {link}` |
| Unresolvable merge conflict | `⚠️ [ACME/android] ACME-123: Merge conflict in ticket files. Branch: {branch}` |
| Tests failing after retries | `❌ [ACME/android] ACME-123: Tests failing after 2 attempts. PR: {link}` |
| Architecture rules violation attempt | `🚨 [ACME/android] ACME-123: Agent attempted to modify arch-rules. Aborted.` |
| Build broken | `🔨 [ACME/android] ACME-123: Build broken. Need help. Branch: {branch}` |
| Successfully merged | `✅ [ACME/android] ACME-123: Merged to {branch}. PR: {pr_url}` |
