...
Real example observed: parallel agents running unnecessary host probing commands like `ls -la /mnt/shared/ubuntu/f/ 2>/dev/null` and environment fingerprinting like `python3 --version && git --version && node --version 2>/dev/null`.
