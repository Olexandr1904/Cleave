...
def test_parse_outcome_analysis_with_ba_returns_default(tmp_path: Path) -> None:
    ...
    orc = _orc()
    assert orc._parse_agent_outcome("analysis", "anything", _ws(tmp_path)) == "default"


def test_parse_outcome_analysis_no_ba_is_unclear(tmp_path: Path) -> None:
    # tmp_path is empty — no BA file
    orc = _orc()
    assert orc._parse_agent_outcome("analysis", "I have questions", _ws(tmp_path)) == "unclear"
    # Even without explicit "unclear"/"questions", missing BA file → unclear (fallback)
    assert orc._parse_agent_outcome("analysis", "all good", _ws(tmp_path)) == "unclear"
...
def test_parse_outcome_qa_pass_from_output(tmp_path: Path) -> None:
    ...
    assert orc._parse_agent_outcome("qa", output, _ws(tmp_path)) == "pass"
...
def test_parse_outcome_qa_fail_from_output(tmp_path: Path) -> None:
    ...
    assert orc._parse_agent_outcome("qa", output, _ws(tmp_path)) == "fail"


def test_parse_outcome_unknown_stage_returns_default(tmp_path: Path) -> None:
    orc = _orc()
    assert orc._parse_agent_outcome("dev", "anything", _ws(tmp_path)) == "default"
    assert orc._parse_agent_outcome("push", "", _ws(tmp_path)) == "default"
