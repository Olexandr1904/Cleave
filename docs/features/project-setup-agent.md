...
| 2026-04-09 | Task 5 review fix: validator handlers now fast-fail with `ToolError` on missing required params (consistent with CRUD handlers); tightened test assertions (exact string match instead of `lower()` substring); added tests for validator missing-param and write-handler invalid-id paths (135 total) |
| 2026-04-09 | Task 6 implemented: `AdminWorkspace` and `AdminWorkspaceState` in `workspace/workspace.py` for non-ticket agent operations (no source/ dir, simple pending/in_progress/completed/failed lifecycle); 7 new tests |
