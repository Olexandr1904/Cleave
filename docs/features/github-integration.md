...
| 2026-04-30 | `GitHubAdapter._run_git` is now async and uses `asyncio.create_subprocess_exec` instead of synchronous `subprocess.run`. The previous implementation blocked the event loop for the duration of each git call (up to `SUBPROCESS_TIMEOUT=300s`) — a noticeable hitch for `push` on large repos and a hard stop on any concurrent advance_workspace work. Callers `create_branch` and `push` now `await self._run_git(...)`. Timeout path properly reaps the killed subprocess. |
| 2026-05-08 | `GitHubAdapter.push` now rewrites the workspace's `origin` URL with the adapter's current token before pushing. Without this, workspaces cloned with an older token kept the stale credential baked into their remote URL, so a fresh `.env` token never reached `git push` and the push failed with a scoping error (e.g. token lacks `workflow` scope). `get_pr_comments` now paginates (per_page=100) so PRs with >100 review comments aren't truncated to the first page. `resolve_comment` GraphQL traversal switched from `pullRequestReview → pullRequest → reviewThreads` to the comment's direct `pullRequestThread` field — works for standalone PR comments that aren't part of a formal review submission, and avoids the 100-thread cap on the previous lookup. |
